...
export function multiply(a: number, { x = 2, y }: {x?: number, y?: number} = {}) {
    return a * x
}

interface DonutChartOptions {
    name?: (xs: [string, number]) => string,  // given d in data, returns the (ordinal) label
    value?: (xs: [string, number]) => number, // given d in data, returns the (quantitative) value
...
export function DonutChart(data: any, {

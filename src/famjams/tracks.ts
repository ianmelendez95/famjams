...
export async function fetchUserTracks(code: string, playlistId: string): Promise<Map<string, Track[]>> {
    const map: Map<string, Track[]> = new Map()

    const playlist: Playlist = await fetchPlaylist(code, playlistId)
